...
class GenClassInfo {
    ...
    constructor(name: string, varName: string, supertype: string, customtype: boolean) {
        this.name = name
...
let exportLangKV = new Map<string, string>()
...
function genCodeCS(handler: FairyEditor.PublishHandler) {
    ...
    let exportClassList = new Array<string>()
    ...
    for (let i: number = 0; i < classCnt; i++) {
        ...
        // 需要绑定的组件变量
        let genClassTypeMap = new Map<string, GenClassInfo>()
        // 需要多语言转义的变量
        let langTypeMap = new Map<string, GenClassInfo>()

        let members = classInfo.members;
        writer.reset();

        writer.writeln('using Game;');
        writer.writeln('using FairyGUI;');
        writer.writeln('using UnityEngine;');
        writer.writeln('using System;');
        writer.writeln();

        let otherPkgName: Array<string> = new Array<string>()
        ...
        for (let index = 0; index < classInfo.members?.Count; index++) {
            ...
            if (element.res) {
                let desc = handler.GetItemDesc(element.res) as FairyGUI.Utils.XML
                ...
                if (element.res.owner.name == handler.pkg.name) {
                    ...
                    if (checkPackageItemIsGenCode(handler, element.res) && element.res.exported) {
                        ...
                        if (otherPkgName.indexOf(className) == -1) {
                            otherPkgName.push(className)
                    ...
                    if (checkOtherPackageItemIsGenCode(element.res)) {
                        ...
                        if (otherPkgName.indexOf(className) == -1) {
                            otherPkgName.push(className)
                            // writer.writeln('import { %s } from "../%s/%s";', className + "_" + element.res.owner.name, element.res.owner.name, className + "_" + element.res.owner.name)
                        }
                    }
                }
            }
        ...
        writer.writeln()
        writer.writeln('public class %s : BaseUI', classInfo.className + "_" + handler.pkg.name);
        writer.startBlock();
        writer.writeln()

        // 写一下这个界面的常量
        writer.writeln('public readonly string URL_ID = "ui://%s%s";', handler.pkg.id, classInfo.resId);
        writer.writeln('private FairyGUI.GComponent com;');
            ...
            if (element.res) {
                ...
                if (element.res.owner.name == handler.pkg.name) {
                    ...
                    if (checkPackageItemIsGenCode(handler, element.res) && element.res.exported) {
                        // 生成BaseUI类
                        let className = "Gen_" + element.res.name + "_" + element.res.owner.name
                        writer.writeln('public %s %s;',className, element.varName)
                    ...
                    // 通过判断在该组件是否勾选了导出，那么也需要生成一下ext类
                        ...
                        if (!superClassName) {
                            // null = GComponent
                            writer.writeln('public FairyGUI.GComponent %s;', element.varName)
                            ...
                            writer.writeln('public FairyGUI.G%s %s;',superClassName, element.varName)
                ...
                // fgui 默认的组件变量即可
                writer.writeln('public FairyGUI.%s %s;',element.type, element.varName)
        ...
        // 构造函数
        writer.writeln('public %s()',classInfo.className + "_" + handler.pkg.name)
        writer.startBlock()
        ...
        writer.save(exportCodePath + '/' + classInfo.className + "_" + handler.pkg.name + '.cs');
...
function genBindClass(handler: FairyEditor.PublishHandler, pkgItem: FairyEditor.FPackageItem) {
    let writer = new CodeWriter();
...
function checkPackageItemIsLang(handler: FairyEditor.PublishHandler, pkgItem: FairyEditor.FPackageItem, refComponentName: string) {
...
function checkPackageItemIsGenCode(handler: FairyEditor.PublishHandler, pkgItem: FairyEditor.FPackageItem) {
...
function checkComponentIsGenCode(handler: FairyEditor.PublishHandler, pkgItem: FairyEditor.FPackageItem, refComponentId: string) {
    let xml = handler.GetScriptData(pkgItem)
...
function checkOtherPackageItemIsGenCode(pkgItem: FairyEditor.FPackageItem) {
    let file = System.IO.File.ReadAllText(pkgItem.file)
    let xml = new FairyGUI.Utils.XML(file)
...
function checkOtherPackageItemExtension(pkgItem: FairyEditor.FPackageItem) {
    let file = System.IO.File.ReadAllText(pkgItem.file)
    let xml = new FairyGUI.Utils.XML(file)
    return xml?.GetAttribute("extention")
}

function logClassInfo(ci: FairyEditor.PublishHandler.ClassInfo) {
